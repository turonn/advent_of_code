def get_directions_and_sideways_stacks_from_file(file)
  directions = []
  sideways_stacks = []
...
def perform_direction(stacks, direction)
  ...
  stacks[direction[2]].concat(transit_crates)
...
def read_top_of_stacks(stacks)
  top_of_stacks = []
  stacks.count.times do |i|
    top_of_stacks << stacks[i + 1].last
  end

  top_of_stacks.join
end

directions, sideways_stacks = get_directions_and_sideways_stacks_from_file('2022/day_5/input.txt')

stacks = rearrange_stacks(sideways_stacks[0..-2])
stacks = follow_directions(stacks, directions)

puts read_top_of_stacks(stacks)