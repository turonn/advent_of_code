class Space
  ...
  attr_accessor :adjacent_rolls, :neighbors
...
class Map
  attr_reader :accessible_rolls

  # @param rows [Array<Array<Space>>]
  # @param row [Array<Space>]
  def initialize(rows)
    @rows = rows
  ...
  def add_neighbors_and_count
    @rows.each_with_index do |row, y|
      row.each_with_index do |space, x|
        ...
        @accessible_rolls += 1 if space.satisfies_condition?
      end
    end
  end
end
...
map.add_neighbors_and_count
puts map.accessible_rolls
