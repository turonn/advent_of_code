...
def get_winning_board_and_last_called(all_bingo_boards, random_numbers)
  random_numbers.each do |pulled_num|
    ...
    winning_board = check_boards_for_winner(all_bingo_boards)

    return [winning_board, pulled_num.to_i] if winning_board.size > 0
  end
end
