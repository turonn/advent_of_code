...
def valid_neighbors(caves, cave, visited_caves)
  ...
  small_caves_visited_twice = visited_caves.select { |c| c[:small] && (visited_caves.count(c) == 2 ) }

  neighbor_caves.reject do |n_c|
    n_c[:name] == 'start' ||
      n_c[:name] == 'end' ||
      (n_c[:small] && (visited_caves.count(n_c) == 2)) ||
      (n_c[:small] && visited_caves.include?(n_c) && !small_caves_visited_twice.empty?)
  end
end
