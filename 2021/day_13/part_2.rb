...
def find_points(points, insts)
  insts.each do |inst|
    points = follow_direction(points, inst)
  end

  points
end

points = find_points(points, insts)
...
trace = { x: x, y: y, type: :scatter, mode: :markers }

plot = Plotly::Plot.new(data: [trace])
plot.generate_html(path: './2021/day_13/line_chart.html', open: false)
